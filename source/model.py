...
from transformers.models.bert.modeling_bert import apply_chunking_to_forward
...
class MLMLoss(nn.Module):
    ...
    def forward(self, res, raw):
        raw_ = deepcopy(raw)
        raw[raw == self.mask_id] = -100
        return f.cross_entropy(res, raw)
...
class NewBertLayer(BertLayer):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            raise NotImplementedError('crossattention not implemented.')
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                raise NotImplementedError('checkpointing is not implemented.')
            else:
